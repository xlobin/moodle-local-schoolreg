...
$courseid = optional_param('courseid', '', PARAM_RAW_TRIMMED);
...
$query = $DB->get_records_sql('SELECT {course}.*, {ls_version}.version from {course} left join {ls_version} on {ls_version}.course_id = {course}.id and {ls_version}.category is null where {course}.category = ? ', array($authenticationinfo['category']));
$results = array();
...
foreach ($query as $row){
    if (isset($listId[$row->id]) && $listId[$row->id] >= $row->version){
        continue;
    }else{
        $results[] = $row;
    }
