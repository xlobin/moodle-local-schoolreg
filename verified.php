...
if (!$unverified = $DB->get_records_sql('select {local_school}.*, {course_categories}.name from {local_school} left join {course_categories} on {local_school}.category = {course_categories}.id where verified = ?', array(1), ($spage * $perpage), $perpage)) {
